...
class Game:
    def __init__(self, vertical_length, horizontal_length, slices,outputSlices,obstacleCells=[]):
        ...
        self.initializeTheGrid(outputSlices,obstacleCells)


    def initializeTheGrid(self,outputSlices,obstacleCells = []):
    ...
    def removeTheAbilityOfRemovingSlice(self, slice,direction: Direction):
        ...
        for cell in slice.cells:
                ...
                # print('this is the color target_cell : ',target_cell.cellColor)
                # print('this is the cell Type : ',target_cell.cellType)
                # print('this is the cell x : ',target_cell.x)
                # Check if the target position is part of the same slice
        ...
        # print('this is the outputSlice : ',outputSlice)
        if(outputSlice is None):
            # raise Exception('outputSlice is Still None!')   
           return      
        for outputCell in outputSlice.cells:
            outputSliceXSet.add(outputCell.x)
            outputSliceYSet.add(outputCell.y)
            
        # print('this is the sliceXSet : ',sliceXSet)
        # print('this is the sliceYSet : ',sliceYSet)
        # print('this is the outputSliceXSet : ',outputSliceXSet)
        # print('this is the outputSliceYSet : ',outputSliceYSet)
        # print('this is the numberOfOutputCells : ',numberOfOutputCells)
        # print('this is the verticalOrHorizontalLengthForComparesion : ',verticalOrHorizontalLengthForComparesion)
        if(
            # numberOfOutputCells >= verticalOrHorizontalLengthForComparesion or
            (sliceXSet.issubset(outputSliceXSet) or sliceYSet.issubset(outputSliceYSet)) #and
            # (outputSlice.cells[0].cellColor != slice.cells[0].cellColor) 
            # (slice.sliceHorizontalLength == outputSlice.sliceHorizontalLength or 
            # slice.sliceVerticalLength == outputSlice.sliceVerticalLength)
          ):
            print('this is the condition :',outputSlice.cells[0].cellColor != slice.cells[0].cellColor)
            for cell in slice.cells:
                old_grid_x = cell.x + 1
                old_grid_y = cell.y + 1
                # Clear the old position by making it an EMPTY cell.
